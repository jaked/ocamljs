...
let rename_append_bytecode oc mapping defined ofs objfile compunit =
  ...
  try
    Jslink.check_consistency objfile compunit;
    List.iter
...
let build_global_target oc target_name members mapping pos coercion =
  ...
  let (instrs, _) =
    Jsgen.compile_implementation target_name lam in
  let rel =
    Emitjs.to_packed_file oc instrs in
  ...
  try
    ...
    let imports =
      List.filter
        ...
        (Jslink.extract_crc_interfaces()) in
    let compunit =
      { cu_name = targetname;
        cu_pos = pos_code;
