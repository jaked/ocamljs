...
let button txt action =
  let b = (doc#createElement "input" : Dom.input) in
  b#_set_type "button";
  b#_set_value txt;
  b#_set_onclick (fun _ -> action (); true);
  b
let toggle_button txt1 txt2 action =
  ...
  b#_set_onclick
    (fun _ ->
       state := not !state;
       b#_set_value (if !state then txt2 else txt1);
       action !state;
       true);
  b
let checkbox txt checked action =
  let b = (doc#createElement "input" : Dom.input) in
  b#_set_type "checkbox";
  b#_set_checked checked;
  b#_set_onclick
    (fun _ -> action b#_get_checked; true);
  let lab = doc#createElement "label" in
  ignore (lab#appendChild b);
  ignore (lab#appendChild (doc#createTextNode txt));
  lab
...
let create_canvas w h =
  let c = (doc#createElement "canvas" : Dom.canvas) in
  c#_set_width w; c#_set_height h; c
...
let load_image src =
  let img = (doc#createElement "img" : Dom.image) in
  lwt_wrap
    (fun c ->
       img#_set_onload c; img#_set_src src)
    >>= fun () ->
  Lwt.return img
...
let shadow texture =
  ...
  *)
    ...
    ctx#_set_globalCompositeOperation "copy";
    ctx#save;
    ctx#scale (8. *. float (w + 2) /. float w) (8. *. float (h + 2) /. float h);
    ctx#translate (-1.) (-1.);
    ctx#drawImage_canvas_ canvas 0. 0.;
    ctx#restore
  in
  ...
  let update_texture lighting phi =
    if lighting then begin
      ...
      let i = truncate (mod_float ((2. *. pi -. phi) *. float w /. 2. /. pi)
                          (float w)) in
      ctx'#drawImage_canvas_ canvas (float i) 0.;
      ctx'#drawImage_canvas_ canvas (float i -. float w) 0.
...
let dv = v' -. v in
*)
ctx#drawImage_canvas9_ shd u v du dv u v du dv;
ctx#restore;
       end
    )
    o.faces
...
let start _ =
  ...
  let canvas' = create_canvas width height in
  ignore (doc#_get_body#appendChild canvas);
  let ctx = canvas#getContext "2d" in
  let ctx' = canvas'#getContext "2d" in
  let r = float width /. 2. in
  let tw = float texture#_get_width in
  let th = float texture#_get_height in
  ...
  let rateText = doc#createTextNode "" in
  let add (d : #Dom.node) (e : #Dom.node) = ignore (d#appendChild e) in
  let ctrl = doc#createElement "div" in
  ctrl#_set_className "controls";
  let d = doc#createElement "div" in
  add d (doc#createTextNode "Click and drag mouse to rotate.");
  add ctrl d;
  let form = doc#createElement "div" in
  let br () = doc#createElement "br" in
  begin
    ...
    let s = (doc#createElement "select" : Dom.select) in
    List.iter
      (fun txt ->
         let o = (doc#createElement "option" : Dom.option) in
         add o (doc#createTextNode txt);
         s#add o (Ocamljs.null ()))
      ["December solstice"; "Equinox"; "June solstice"];
    s#_set_onchange
      (fun _ ->
         let o =
           match s#_get_selectedIndex with
               0 -> obliquity
             | 1 -> 0.
             | _ -> -. obliquity
         in
  ...
  begin
    ...
    add form (br ());
    add form (doc#createTextNode "Frames per second: ");
    add form rateText
  end;
  add ctrl form;
  add (doc#_get_body) ctrl;
  let p = doc#createElement "p" in
  p#_set_innerHTML
  ...
  let mx = ref 0 in
  let my = ref 0 in
  canvas#_set_onmousedown
    (fun ev ->
       mx := ev#_get_clientX; my := ev#_get_clientY;
       let c1 =
         (fun ev ->
            let x = ev#_get_clientX and y = ev#_get_clientY in
            ...
            mx := x; my := y) in
       doc#addEventListener_mouseEvent_ "mousemove" c1 true;
       let rec c2 _ =
         doc#removeEventListener_mouseEvent_ "mousemove" c1 true;
         doc#removeEventListener "mouseup" c2 true in
       doc#addEventListener "mouseup" c2 true;
       false);
  let ti = ref ((Javascript.new_Date ())#getTime) in
  let fps = ref 0. in
  ...
  let rec loop t phi =
    ...
    if !clipped then begin
      ctx'#beginPath;
      ctx'#arc r r (r *. 0.95) 0. (-. 2. *. pi) true;
      ctx'#clip
    end;

    ctx'#setTransform (r -. 2.) 0. 0. (r -. 2.) r r;
    ctx'#_set_globalCompositeOperation "lighter";
...
*)
  ...
  in
  loop ((Javascript.new_Date ())#getTime) 0.
)

let _ =
Dom.window#_set_onload start
